  ...
  useEffect(()=> {
    const loadModel = async () => {
      try {
        await tf.ready()
          const modelUrl =
            'https://raw.githubusercontent.com/9saurabh9/ReadParkingSigns-JSModel/refs/heads/master/model.json'
          const model = await tf.loadLayersModel(modelUrl, { onProgress: (p: number) => {
            setProgress(p)
            console.log(p)
            } 
          })
